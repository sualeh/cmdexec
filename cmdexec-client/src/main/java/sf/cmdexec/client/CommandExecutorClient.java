...
public abstract class CommandExecutorClient
{
  ...
  public CommandExecutorClient(final String host, final int port)
    throws Exception
  {
    this.host = host;
    this.port = port;
  ...
  public List<CommandResult> executeCommands(final Command... commands)
    throws Exception
  {
    return executeCommands(Arrays.asList(commands));
  }

  protected List<CommandResult> executeCommands(final List<Command> commands)
    throws Exception
  {
    final CommandExecutorService cmdExecSvc = locateService();
    log(String.format("Executing commands: %s", commands));
    final List<CommandResult> commandResults = cmdExecSvc
    ...
    throws Exception
  {
    ...
    final boolean test = testImplementation(testCode) == cmdExecSvc
      .test(testCode);
    log(String.format("Connection test is %ssuccessful", (test? "": "NOT ")));
    return test;
  }

  private CommandExecutorService locateService()
    throws RemoteException, NotBoundException
  {
    try
    {
      ...
      log(String.format("Connecting to RMI server %s:%d from %s",
                        server,
                        port,
                        address));
    }
